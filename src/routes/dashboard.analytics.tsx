...
function Analytics() {
  ...
  const fatigueBuckets = [
    {
      name: "Low (<40)",
      value: snapshot.volunteers.filter((volunteer) => volunteer.fatigue < 40).length,
    },
    {
      name: "Medium (40-70)",
      value: snapshot.volunteers.filter(
        (volunteer) => volunteer.fatigue >= 40 && volunteer.fatigue < 70,
      ).length,
    },
    {
      name: "High (>=70)",
      value: snapshot.volunteers.filter((volunteer) => volunteer.fatigue >= 70).length,
    },
  ];
  const zoneLoad = snapshot.zones.map((zone) => ({
    ...
    <div className="mx-auto max-w-7xl px-6 py-8">
      <header className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          Operations, fatigue distribution, and zone performance.
        </p>
      </header>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card title="Incidents & volunteers · last 24h">
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={snapshot.hourlyLoad}>
              <defs>
        ...
        <Card title="Avg response time · per zone (min)">
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={snapshot.zonePerformance}>
              <CartesianGrid stroke="oklch(0.88 0.015 95)" strokeDasharray="3 3" />
